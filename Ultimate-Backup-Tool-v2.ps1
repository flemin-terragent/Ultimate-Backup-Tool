...
Param([Parameter(Position=0,Mandatory=$False,
      ...
      [ValidateNotNullorEmpty()]
       ...
       $file1 = ("$fileW\$BackupFolder\$WeekVMnames\Virtual Hard Disks"),
       ...
       $file2 = ("$fileM\$BackupFolder\$WeekVMnames\Virtual Hard Disks"),
      ...
      #$VhdSizeM = @(Get-VM $MonthVMnames  | Select-Object VMId | Get-VHD –Property @{label=’Size(GB)’;expression={$_.filesize/1gb –as [int]}}),
      $size1= (Get-Item ($file1).length | Out-Default),
      $size2= (Get-Item ($file2).length | Out-Default),
      $VhdSizeM = (Get-VM -Name $MonthVMnames | Select-Object VMId | Get-VHD | Select -Property FileSize | Out-Default),
      $VhdSizeW = (Get-VM -Name $WeekVMnames | Select-Object VMId | Get-VHD | Select -Property FileSize | Out-Default),
...
Write-Host "Export Failed." -ForegroundColor Red
      ...
      #   if($VhdSizeM -ieq $size){
   # Write-Host "Export script finished." -ForegroundColor Green
#}
#else{
   # Write-Host "ExportFailed"  -ForegroundColor Red
#}
    } #close foreach
